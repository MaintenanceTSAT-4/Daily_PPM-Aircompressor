// ============================================================
// Dashboard + Approvals + shared detail modal (Google Sheet API)
// ============================================================

async function fetchChecks(filter = {}) { return await API.listChecks(filter); }
async function countPending() { return await API.countPending(); }
...
const Dashboard = (() => {
  async function render() {
    ...
    drawTable(all);
    drawLineCards(machines.filter(m => truthy(m.is_active)), all);
  }
  ...
  function drawLineCards(machines, all) {
    ...
    grid.innerHTML = machines.map(m => {
      const todays = all.filter(r => r.machine_id === m.id && String(r.check_date) === today);
...
const Detail = (() => {
  ...
  async function open(id) {
    ...
    const dc = b.header, vals = b.values || [], sigs = b.signatures || [];
    ...
    root.innerHTML = `
      <div class="modal-bg" id="mbg">
        <div class="modal">
          ...
          <h2>${dc.machine_code} — ${dc.machine_name}</h2>
          ...
          <tbody>${vals.map(v=>`<tr>
            <td>${v.label}</td>
            <td>${displayVal(v)}</td>
            <td>${truthy(v.is_normal)?'<span class="tag-normal">ปกติ</span>':'<span class="tag-abnormal">ผิดปกติ</span>'}</td>
            <td class="small">${v.note||""}</td>
          </tr>`).join("")}</tbody></table>

          <div class="sig-grid" style="margin-top:16px">
            <div class="sig-box"><strong>ผู้ตรวจ (Operator)</strong>${sigView(opS)}</div>
            <div class="sig-box"><strong>ผู้อนุมัติ (Approver)</strong>${sigView(apS)}</div>
          </div>
  ...
  async function decide(dc, status) {
    const reason = (document.getElementById("rejReason")||{}).value ? document.getElementById("rejReason").value.trim() : "";
    if (status === "rejected" && !reason) return toast("กรุณาระบุเหตุผลการตีกลับ", "err");
    let signerName = "";
    if (status === "approved") {
      const sig = apprSig.getData();
      if (!sig) return toast("กรุณาลงลายเซ็นผู้อนุมัติ (พิมพ์ชื่อ) ก่อน", "err");
      signerName = sig.signer_name;
    }
    try {
      await API.decide(dc.id, status, reason, signerName);
...
function sigView(s){
  ...
  return `<div class="sig-saved"><div style="font-family:cursive;font-size:20px">${s.signer_name}</div>
    <div class="small">${s.signer_name}<br><span class="muted">${fmtDateTime(s.signed_at)}</span></div></div>`;
}
